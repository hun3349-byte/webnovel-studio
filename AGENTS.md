...
│   │   │   └── server.ts                    # 서버 클라이언트
│   │   ├── ai/
│   │   │   ├── Codex-client.ts             # Codex API 클라이언트
│   │   │   └── prompts/
│   │   │       ├── base-persona.ts
...
3. AI 에피소드 생성 (Codex API SSE 스트리밍)
...
- [x] **Codex API 클라이언트** (`src/lib/ai/Codex-client.ts`) - SSE 스트리밍 + TTFB 방어
...
- [x] **Codex API 파라미터 최적화** (`src/lib/ai/Codex-client.ts`)
  - temperature: 0.8 → 0.82로 상향 (문장의 다양성과 자연스러움 향상)
  - 권장 범위: 0.75~0.85
  - 참고: Codex API는 `frequency_penalty` 미지원 (OpenAI 전용)
...
- [x] **🔧 V9.6 패치** — 문단 끊어쓰기 과도 수정
  - **문제점**: 한 문장마다 빈 줄을 넣어 문단이 과도하게 쪼개짐 (소설이 시(詩)처럼 보임)
  - **원인**: 가독성 규칙 "문단은 2~3문장"을 AI가 "1문장도 OK"로 해석
  - **STYLE_DNA 서술 원칙 수정**
    - 기존: "대사 앞뒤로 빈 줄. 문단은 2~3문장."
    - 변경: "문단은 2~4문장으로 구성. 한 문장짜리 문단은 극적 강조 시에만 사용 (1화에 최대 3회)."
    - 추가: "서술/묘사/행동은 같은 흐름이면 한 문단으로 묶어라. 매 문장마다 줄바꿈하지 마라."
  - **STYLE_DNA에 나쁜 예/좋은 예 추가**
    - × (매 문장마다 줄바꿈 — 시처럼 보임): "눈을 떴다.\n\n시야가 흐렸다.\n\n천장이 보였다."
    - ○ (2~4문장을 한 문단으로): "눈을 떴다. 시야가 흐렸다. 천장이 보였다. 낮고 어두운 천장에서..."
    - ★ 한 문장 독립 문단은 극적 순간에만 (1화에 최대 3번): "이 손은 내 손이 아니었다."

- [x] **🔧 V9.7 패치** — 네이버 모바일 가독성 최적화
  - **목표**: 모바일(스마트폰) 환경에서 읽기 좋은 포매팅 강제
  - **STYLE_DNA 네이버 모바일 가독성 섹션 신규 추가**
    - 1. [문단 길이] 하나의 문단은 2~3문장으로 구성. 4문장 이상 이어붙이지 마라.
    - 2. [문단 사이 빈 줄] 모든 문단 사이에 빈 줄(엔터 2번) 필수 삽입.
    - 3. [대사 독립] 대사(" ")는 앞뒤로 빈 줄을 넣어 시각적으로 독립시켜라.
    - 4. [장면/시점 전환] 장소나 시간이 바뀔 때는 빈 줄 3개 또는 *** 구분자 사용.
    - 5. [하이라이트 문장] 반전/깨달음/충격의 순간은 짧은 문장을 단독 배치.

- [x] **🔧 V9.8 패치** — 문장 호흡 + 모바일 가독성 균형
  - **문제점**: V9.7에서 문장이 너무 짧게 끊어져 스타카토 문체가 됨
  - **STYLE_DNA 문장 호흡 규칙 강화**
    - 같은 흐름의 동작/감각은 접속사(~하자, ~하며, ~하고)로 연결
    - 짧은 문장(1~3어절) 3개 이상 연속 금지
    - 2~3개 동작을 하나의 문장으로 묶어 호흡을 길게 유지
  - **스타카토 vs 복문 연결 예시**
    - × "노인을 눕혔다. 힘이 빠졌다. 소리가 났다." (스타카토 금지)
    - ○ "노인을 눕히자 팔에서 힘이 빠지며 털썩, 둔탁한 소리가 울렸다." (복문 권장)
  - **문단 구분 규칙 명확화**
    - 빈 줄은 '흐름의 전환' 시에만 사용 (시선/시간/장소/분위기 변화)
    - 같은 인물의 연속 동작, 원인→결과 연속은 빈 줄 없이 연결
  - **대사 포매팅 유지**
    - 대사 앞뒤로 빈 줄 삽입하여 시각적 독립

- [x] **🧬 V10.0 메이저 업데이트** — Style Evolution Engine (문체 진화 엔진)
  - **핵심 개념**: AI 작가가 쓸수록 필력이 성장하는 자가진화 시스템
  - **아키텍처**:
    - 레퍼런스 소설 분석 → StyleDNA 추출
    - PD 피드백 학습 → DNA 업데이트 (가중치 2배)
    - 동적 프롬프트 주입 → 프로젝트별 맞춤 문체
  - **DB 마이그레이션** (`supabase/migrations/00009_style_dna.sql`)
    - `style_dna` 테이블: 개별 StyleDNA (레퍼런스/PD피드백/수동)
    - `style_dna_merged` 테이블: 프로젝트별 합성 DNA
    - `episodes.original_content` 컬럼: AI 원본 저장 (피드백 학습용)
  - **Core 모듈** (`src/core/style/`)
    - `style-analyzer.ts`: Codex API 기반 문체 분석
    - `style-dna-manager.ts`: DNA CRUD + 가중치 합성
    - `style-injector.ts`: 동적 STYLE_DNA 프롬프트 생성
    - `feedback-learner.ts`: PD 피드백 자동 학습
  - **API 엔드포인트**
    - `POST /api/ai/analyze-style`: 레퍼런스 텍스트 분석
    - `GET/POST /api/projects/[id]/style-dna`: StyleDNA 목록/추가
    - `GET/PATCH/DELETE /api/projects/[id]/style-dna/[dnaId]`: 개별 DNA CRUD
    - `POST /api/projects/[id]/style-dna/merge`: 합성 DNA 재생성
    - `GET /api/projects/[id]/style-dna/merged`: 합성 DNA 조회
  - **프롬프트 통합**
    - `buildEpisodeGenerationPrompts()` async 변환 + projectId 파라미터
    - 하드코딩된 STYLE_DNA → `buildDynamicStyleDNA(projectId)` 동적 로드
    - 기존 Writing Memory 시스템 완전 대체
  - **자동 학습 트리거**
