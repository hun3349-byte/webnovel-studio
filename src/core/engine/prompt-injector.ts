...
4. [분량 및 엔딩] 4,000~6,000자 엄수. 마지막은 반드시 절단신공(위기/발견/반전/충격적 선언)으로 끊어라.
...
<style_dna>
[서술 원칙]
- 하드보일드 & Show, Don't Tell. 철학적 독백을 버리고, 감정을 직접 쓰지 말고 '신체 반응'으로 보여줘라.
- 대사 비율은 전체의 20% 이하. 대사(" ") 앞뒤로는 반드시 빈 줄 삽입. 한 문단은 최대 2~3문장.
...
[대사 톤]
유들유들한 능구렁이. 도발에 정색하지 않고 너스레와 팩트 폭격으로 맞받아친다.
</style_dna>
`;
...
export function buildCompactCharacterSection(characters: ActiveCharacter[]): string {
  ...
  const compactChars = characters
    ...
    .map(c => {
      ...
      return `- ${c.name} [${roleLabel}]: ${c.personality?.substring(0, 30) || '성격 미설정'}... ${status ? `(${status})` : ''}`;
    })
    .join('\n');
...
export function buildCompactWorldSection(worldBible: SlidingWindowContext['worldBible']): string {
  ...
  const rules = Array.isArray(worldBible.absolute_rules)
    ? worldBible.absolute_rules.slice(0, 3).join(', ')
    : typeof worldBible.absolute_rules === 'string'
      ? worldBible.absolute_rules.substring(0, 100)
      : '';

  return `
<world_bible>
- 세계관: ${worldBible.world_name || '미설정'} (${worldBible.time_period || '시대 미설정'})
- 힘의 체계: ${worldBible.power_system_name || '없음'}
- 핵심 규칙: ${rules || '없음'}
- 금기: ${worldBible.forbidden_elements?.slice(0, 3).join(', ') || '없음'}
</world_bible>
`;
}
